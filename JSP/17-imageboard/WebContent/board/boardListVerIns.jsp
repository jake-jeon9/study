...
#currentpaging{ color: yellow; text-decoration: unerline; }
...
function action(){
	var sl = document.getElementsByName("chooseGetList")[0].value;
		location.href="boardListVerIns.jsp?pg=1&sl="+sl;
}
...
리스트 갯수 보기<select name="chooseGetList" onchange="action()">
	...
	<%
		...
		<tr bgcolor="ffffcc" align="center">
			...
			<td><a href="#" id="subjectAQ" 
			onclick="isLogin(<%=boardDTO.getSeq() %> )">
	...
	<td colspan="5" align="center">
		<%if(startPage>3){ %>
		[<a id="paging" href="boardListVerIns.jsp?pg=<%=startPage-1%>">이전]</a>
		<%} %>
		<% for(int i=startPage; i<=endPage;i++){ %>
			<% if(	i==pg  ){//현재페이지 %>
			[<a id="currentpaging" href="boardListVerIns.jsp?pg=<%=i%>"><%=i %>]</a>
		<%}else{ //현재페이지 아님 %>
		[<a id="paging"  href="boardListVerIns.jsp?pg=<%=i%>"><%=i %>]</a>
		<%} %>
		<%} %>
		<% if(endPage<totalP){ %>
		[<a id="paging"  href="boardListVerIns.jsp?pg=<%=endPage+1%>">다음]</a>
		<%} %>
		</td>
	</tr>
	</table>
